...
> To design decision support models that can be used by transportation authorities to .content-box-gray[select the routes to subsidise] and to .content-box-gray[design auction with appropriate requitrements for these routes]
...
.right-column[
.gray[
RQ1.  How to model the selection of routes to subsidise?  

RQ2.  How to model an auction that replicates the selection of airlines to serve subsidised routes?
]

G1. To develop a model that can compare existing and non-existing subsidised schemes, and can objectively select an optimal set of subsidised routes.

G2. To develop a model that can help the transportation authorities choose appropriate tendering requirements by studying how changes in these requirements may affect the subsidies, the airlines, and the passengers.
]
...
---
class: hover
# Models for Procuring Subsidised routes

.left-column[
### .marron-bold[Selecting subsidised routes]
]
.right-column[
 * Add picture
]
...
### .marron-bold[Auction for Subsidised routes] 
]
.right-column[
 .pull-left[
1. Bid Preparation
...
2. Closure of an airport (e.g., Stockholm Bromma Airport) with commercial traffic may require a subsidised route to as replacement
]
