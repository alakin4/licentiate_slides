...
* Impaired accessibility to certain destination
...
- This led to governments deregulating air transportation, meaning airlines could freely choose new profitable routes and ignore/abandon the unprofitables ones
- As a result, profit oriented airlines ignore routes that serve remote regions if they have insufficient pax demand, which usually translates into unprofitability
- In a nutshell, dereguration impairs accessibility of people in small communities or remote regions to and from given destinations e.g., a mojor city, a hospital, and a hub or an international airport within a specified travel time. 
- This is a bad thing because it is not fair that part of the population may not access other regions and air transportation would provided their best chance. and even so, connecting these regions may have economic benefits for the whole country. crucial for economic social devpt
- To ensure accessibility to and from these regions, transportation authorities in Europe , the USA, Asia, and Austraria have these subsidy schemes where they procure air services: They pay subsidies to airlines who in turn provide air services.
---
class: slide-font
...
> To design decision support models that can be used by transportation authorities to .content-box-gray[select the routes to subsidise] and to .content-box-gray[design an auction with appropriate requitrements for these routes]
...
1. To develop a model that can can objectively select an optimal set of subsidised routes.
...
1. Develop a .content-box-gray[budget-constrained optimisation model] that selects an optimal set of subsidised routes from all possible routes based on givenaccessibility criteria.

--

2.  Propose a method to .content-box-gray2[estimate the subsidies of new routes]. This makes it possible to assesses all the possible subsidised routes.
...
4.  Develop a .content-box-gray2[bid-preparation model as an optimisation model] that is specific  to  subsidised  routes,  to  represent  the  airlines’  behaviour  in  the auction framework. The bidding airline is interested in minimising the subsidies subject to a target profit threshold.
...
6.  .content-box-gray2[Estimate the route operation cost for the small aircraft types] that typically operate subsidised routes.  A model based on the small aircraft types—used to operate the subsidised routes—gives a more accurate cost estimate than a model based on bigger aircraft types,  which are more appropriate for commercially viable routes.
...
.pull-right[
- $y_p = 1$ if center $p$ is corvered; 0 otherwise

- $x_f = 1$ if route $f$ selected; 0 otherwise

- Maximise number of people with respect to an accessibility target $t$

- A budget $B$ should not be exceeded

- Ground transportation, commercial route, or subsidised route
]
...
- We develop an integrated auction framework that starts with auction definition by the transportation authority. Here, the requiremnest of the maximum airfares and minimum number of flights are specified.
...
.pull-right[
- Minimise total subsidy $s$ per bundle 

- Capture demand elasticity using a utility function: $d_a=D\frac{e^{U_{a}}}{1+e^{U_{a}}}$, $U_{a}(time, airfare)$
- Estimate route operating cost: $C(seat, distance)$

- Balance demand and capacity

- Profit threshold $\psi$
]
---
class: slide-font_2
## An integrated auction framework
...
.pull-right[
- Minimise total subsidy for serving all regions

- $x_b = 1$ if bundle $b$ is selected; 0 otherwise

- Each airline is wins atmost one bundle
]
---
class: slide-font_2, hover
...
- No restriction on air fare and number of daily flights is good for the government
...
- Solve the two task of the procurement process simultaneously

- Enhanced flight-scheduling component in the bid preparation model

- Re-designing subsidised routes based on electric
